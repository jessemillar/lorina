...
this.pullTo=function(a,b,c){a-=this.x;var d=b-this.y;b=Math.sqrt(a*a+d*d);d=d/b*c;1<b&&(this.pushHorizontal(a/b*c),this.pushVertical(d));return this};this.pullToward=function(a,b){if(!a.deleted){var c=a.x-this.x,d=a.y-this.y,e=Math.sqrt(c*c+d*d),d=d/e*b;1<e&&(this.pushHorizontal(c/e*b),this.pushVertical(d))}return this};this.bounce=function(a,b,c,d){a||b||c||d||(a=0,b=l.globals.room.width,c=0,d=l.globals.room.height);this.x+this.bound.x<=a?(this.x=a-this.bound.x,this.momentum.x=-this.momentum.x):
this.x+this.bound.x+this.bound.width>=b&&(this.x=b-this.bound.width-this.bound.x,this.momentum.x=-this.momentum.x);this.y+this.bound.y<=c?(this.y=c-this.bound.y,this.momentum.y=-this.momentum.y):this.y+this.bound.y+this.bound.height>=d&&(this.y=d-this.bound.height-this.bound.y,this.momentum.y=-this.momentum.y);return this};this.applyPhysics=function(){if(this.momentum.x||this.momentum.y)Math.abs(this.momentum.x)>Math.abs(this.momentum.y)?(this.friction.x=this.friction.movement,this.friction.y=Math.abs(this.momentum.y/
