...
(this.x=b-this.bound.width-this.bound.x,this.momentum.x=-this.momentum.x);this.y+this.bound.y<=c?(this.y=c-this.bound.y,this.momentum.y=-this.momentum.y):this.y+this.bound.y+this.bound.height>=d&&(this.y=d-this.bound.height-this.bound.y,this.momentum.y=-this.momentum.y);return this};this.applyPhysics=function(){this.momentum.x&&this.momentum.y&&(Math.abs(this.momentum.x)>Math.abs(this.momentum.y)?(this.friction.x=this.friction.movement,this.friction.y=Math.abs(this.momentum.y/this.momentum.x*this.friction.movement)):
(this.friction.x=Math.abs(this.momentum.x/this.momentum.y*this.friction.movement),this.friction.y=this.friction.movement));0!==this.momentum.x&&(this.moveHorizontal(this.momentum.x),0>this.momentum.x?(this.momentum.x+=this.friction.x,0<this.momentum.x&&(this.momentum.x=0)):0<this.momentum.x&&(this.momentum.x-=this.friction.x,0>this.momentum.x&&(this.momentum.x=0)));0!==this.momentum.y&&(this.moveVertical(this.momentum.y),0>this.momentum.y?(this.momentum.y+=this.friction.y,0<this.momentum.y&&(this.momentum.y=
0)):0<this.momentum.y&&(this.momentum.y-=this.friction.y,0>this.momentum.y&&(this.momentum.y=0)));0!==this.momentum.rotation&&(this.degree+=this.momentum.rotation,0>this.momentum.rotation?(this.momentum.rotation+=this.friction.rotation,0<this.momentum.rotation&&(this.momentum.rotation=0)):0<this.momentum.rotation&&(this.momentum.rotation-=this.friction.rotation,0>this.momentum.rotation&&(this.momentum.rotation=0)));return this}};var Finger=function(){this.database=[];var a=this;document.touchstart=function(b){a.touches(b)};document.touchmove=function(b){a.touches(b)};document.touchend=function(b){a.touches(b)};document.touchcancel=function(b){a.touches(b)};this.touches=function(a){a?this.database=a.touches:this.database.length=0};this.checkTouched=function(a){if(0<this.database.length)for(var c=this.database.length;c--;){if(this.database[c].pageX<a.bound.x+a.bound.width&&this.database[c].pageX>a.bound.x&&this.database[c].pageY<
...
l.dom.height=window.innerHeight;this.stretch&&(l.room.width=l.dom.width,l.room.height=l.dom.height);return this};this.makeRoomFullscreen=function(){this.stretch=!0;return this};this.hideCursor=function(){l.dom.style.cursor="none";return this};this.start=function(a){this.setRoom(a);return this};this.stop=function(){clearInterval(this.loop);return this};this.setRoom=function(a){this.loop&&clearInterval(this.loop);this.loop=setInterval(a,1E3/60);return this};this.setColor=function(a){this.color=a;return this};
this.blank=function(a){l.ctx.fillStyle=a?a:this.color;l.ctx.fillRect(0,0,l.dom.width,l.dom.height);l.buffer.length=0;return this};this.draw=function(){l.buffer.sort(function(a,c){return a.y-c.y});for(var a=l.buffer.length;a--;)l.buffer[a].draw();return this};this.checkSolid=function(a,b){var c=this.checkCollision(a,b);c&&(a.previous.y+a.bound.y+a.bound.height<=c.y+c.bound.y?a.y=c.y+c.bound.y-(a.bound.height+a.bound.y):a.previous.y+a.bound.y>=c.y+c.bound.y+c.bound.height?a.y=c.y+c.bound.y+c.bound.height-
a.bound.y:a.previous.x+a.bound.x+a.bound.width<c.x?a.x=c.x+c.bound.x-(a.bound.width+a.bound.x):a.previous.x+a.bound.x>c.x&&(a.x=c.x+c.bound.x+c.bound.width-a.bound.x));return this};this.checkCollision=function(a,b){if(a.deleted)return!1;if(b.database){for(var c=b.database.length;c--;)if(!b.database[c].deleted&&a.x+a.bound.x<b.database[c].x+b.database[c].bound.x+b.database[c].bound.width&&a.x+a.bound.x+a.bound.width>b.database[c].x+b.database[c].bound.x&&a.y+a.bound.y<b.database[c].y+b.database[c].bound.y+
...
function(){this.y=this.x=null};this.clicked=function(){1==event.which?this.leftClick=!0:2==event.which?this.middleClick=!0:3==event.which&&(this.rightClick=!0,event.preventDefault())};this.checkLeftClicked=function(a){return this.leftClick&&this.x<a.bound.x+a.bound.width&&this.x>a.bound.x&&this.y<a.bound.y+a.bound.height&&this.y>a.bound.y?!0:!1};this.checkMiddleClicked=function(a){return this.middleClick&&this.x<a.bound.x+a.bound.width&&this.x>a.bound.x&&this.y<a.bound.y+a.bound.height&&this.y>a.bound.y?
!0:!1};this.checkRightClicked=function(a){return this.rightClick&&this.x<a.bound.x+a.bound.width&&this.x>a.bound.x&&this.y<a.bound.y+a.bound.height&&this.y>a.bound.y?!0:!1};this.cancel=function(){1==event.which?this.leftClick=!1:2==event.which?this.middleClick=!1:3==event.which&&(this.rightClick=!1,event.preventDefault())}};var Pencil=function(){this.setPosition=function(a,b){this.x=a;this.y=b;return this};this.setSize=function(a,b){this.width=a;this.height=b;return this};this.setRadius=function(a){this.radius=a;return this};this.setColor=function(a){this.color=a;return this};this.setStroke=function(a){this.stroke=a;return this};this.setArc=function(a,b){this.start=a;this.stop=b;return this};this.strokeCircle=function(){l.ctx.strokeStyle=this.color;l.ctx.lineWidth=this.stroke;l.ctx.beginPath();l.ctx.arc(Math.round(this.x),
Math.round(this.y),this.radius,0,2*Math.PI);l.ctx.stroke();return this};this.fillCircle=function(){l.ctx.fillStyle=this.color;l.ctx.beginPath();l.ctx.arc(Math.round(this.x),Math.round(this.y),this.radius,0,2*Math.PI);l.ctx.fill();return this};this.strokeArc=function(){l.ctx.strokeStyle=this.color;l.ctx.lineWidth=this.stroke;l.ctx.beginPath();l.ctx.arc(Math.round(this.x),Math.round(this.y),this.radius,this.start*Math.PI/180,this.stop*Math.PI/180);l.ctx.stroke();return this};this.fillArc=function(){l.ctx.fillStyle=
this.color;l.ctx.beginPath();l.ctx.arc(Math.round(this.x),Math.round(this.y),this.radius,this.start*Math.PI/180,this.stop*Math.PI/180);l.ctx.fill();return this};this.strokeRectangle=function(){l.ctx.strokeStyle=this.color;l.ctx.lineWidth=this.stroke;l.ctx.beginPath();l.ctx.rect(Math.round(this.x),Math.round(this.y),Math.round(this.width),Math.round(this.height));l.ctx.stroke();return this};this.fillRectangle=function(){l.ctx.fillStyle=this.color;l.ctx.beginPath();l.ctx.rect(Math.round(this.x),Math.round(this.y),
Math.round(this.width),Math.round(this.height));l.ctx.fill();return this};this.fillPie=function(a){100>Math.round(a)?(a=270+3.6*a,360<a&&(a-=360),l.ctx.fillStyle=this.color,l.ctx.beginPath(),l.ctx.moveTo(Math.round(this.x),Math.round(this.y)),l.ctx.arc(Math.round(this.x),Math.round(this.y),this.radius,270*Math.PI/180,a*Math.PI/180),l.ctx.lineTo(Math.round(this.x),Math.round(this.y)),l.ctx.fill()):this.fillCircle();return this}};var Speaker=function(){this.file=new Audio;this.setFile=function(a){this.file.src=a;return this};this.pause=function(){this.file.pause();return this};this.play=function(){this.file.play();var a=this;this.file.onended=function(){a.file.currentTime=0};return this};this.loop=function(){this.file.loop=!0;this.file.play();return this}};var Timer=function(){this.log=function(){this.time=new Date;return this};this.compareTo=function(a){return(Date.parse(this.time)>Date.parse(a.time)?Date.parse(this.time)-Date.parse(a.time):Date.parse(a.time)-Date.parse(this.time))/1E3}};var Tool=function(){this.random=function(a,b){return Math.random()*(b-a)+a};this.measureX=function(a,b){return Math.abs(a.x-b.x)};this.measureY=function(a,b){return Math.abs(a.y-b.y)};this.measureDistance=function(a,b){var c=this.measureX(a,b),d=this.measureY(a,b);return Math.sqrt(c*c+d*d)};this.measureAngle=function(a,b){var c=b.x-a.x,d=a.y-b.y,e=180*Math.atan(d/c)/Math.PI;0>c?e+=180:0<c&&0>d&&(e+=360);return e}};var Typewriter=function(){this.setOpacity=function(a){this.opacity=a;return this};this.setStyle=function(a){this.style=a;return this};this.setFont=function(a){this.font=a;return this};this.setSize=function(a){this.size=a;return this};this.setAlignment=function(a){this.alignment=a;return this};this.setColor=function(a){this.color=a;return this};this.hud=function(){this.textMode="hud";return this};this.setPosition=function(a,b){this.x=a;this.y=b;return this};this.write=function(a){var b=this.style?
this.style:"";if(this.size){var e=this.size;this.y+=this.size}else e=10,this.y+=10;l.ctx.font=b+" "+e+"px "+(this.font?this.font:"sans-serif");l.ctx.fillStyle=this.color?this.color:"#000000";l.ctx.textAlign=this.alignment?this.alignment:"left";this.opacity&&(l.ctx.globalAlpha=this.opacity);this.print(a);l.ctx.globalAlpha=1;return this};this.print=function(a){"hud"==this.textMode?l.ctx.fillText(a,this.x,this.y):l.ctx.fillText(a,this.x-l.camera.x,this.y-l.camera.y);this.textMode="world"};var a=!1,b=
0;this.type=function(c,d){a||(this.key(c,d),a=!0);this.write(c.substr(0,b));return this};this.key=function(a,d){for(var e=0;e<a.length;e++)setTimeout(function(){b++},d*e)}};